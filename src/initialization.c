...
int setLoadNames(int num_loads, char ***load_names, int index, int argc, char **argv) {
	int i;
	if (num_loads <= 0 || num_loads > (argc - index)) num_loads = argc - index;
...
int parseConfig(char *config_buffer, int config_filesize) {
	...
	if (flag != GOOD) {
		if (MyRank == ROOT) EmitLog(MyRank, SCHEDULER_THREAD, "Invalid/incomplete configuration file. Using default values where needed.", -1, PRINT_ALWAYS);
		ret = BAD;
	}
	return ret;
}
