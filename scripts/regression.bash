...
export BUILDDIR=${HOME}/working/systemburn
...
case ${SHORTHOSTNAME} in
	...
	chester*)
		...
		export SYSTEMBURN="systemburn-mpi systemburn-shmem"
...
export BOILER='^$|Initialization complete|Elapsed time for this load|Run Completed|^[ 0-9 ][ 0-9]*$'
...
export PLANLB='^   DGEMM|^   RDGEMM|^   [DL]STREAM|^   [DL]STRIDE|^   FFT[12]D|^   GUPS|^   PV[123]|^   SLEEP'
...
passfail() {
	...
	ERRORCOUNT=$(cat r.${OUTPUT}.stdout | egrep -v "$BOILER" | egrep -v "$TEMPER" | egrep -v "$LDTOP" | egrep -v "$LDBOT" | egrep -v "$PLANLB" | egrep -v "$RUNTIMETAGLINE" | wc -l)
	if [ "${ERRORCOUNT}" -ne "0" ]
	then
		PASS=false
	fi
...
genload $TIME $PROCESSORS_PER_NODE $CORES_PER_PROCESSOR PV4	"$PV4"
genload $TIME $PROCESSORS_PER_NODE $CORES_PER_PROCESSOR RDGEMM	"$RDGEMM1"	
genload $TIME $PROCESSORS_PER_NODE $CORES_PER_PROCESSOR RDGEMM	"$RDGEMM2"
...
do
	...
	do
		...
		do
		  ${MPI_INCANTATION} ./${SYSTEMBURNEXE} -c ${COMMTEST} ${LOAD}   > r.${SYSTEMBURNEXE}_${LOAD}-c${COMMTEST}.stdout       2> r.${SYSTEMBURNEXE}_${LOAD}-c${COMMTEST}.stderr
		  echo "$?"                                                > r.${SYSTEMBURNEXE}_${LOAD}-c${COMMTEST}.status
		  passfail ${SYSTEMBURNEXE}_${LOAD}-c${COMMTEST}
		done
...
then
	...
	then
		mail -s "SystemBurn Regression Failure on ${SHORTHOSTNAME}" ${MAILREPORT} < report
	else
		mail -s "SystemBurn Regression Succeeded on ${SHORTHOSTNAME}" ${MAILREPORT} < report
	fi
fi
#cp output* $WORKDIR
#cd ..
