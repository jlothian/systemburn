 ...
 * \sa killPV3Plan
 */
void *makePV3Plan(data *m){
    Plan *p;
    PV3data *d;
    p = (Plan *)malloc(sizeof(Plan));
    assert(p);
    if(p){
        p->fptr_initplan = &initPV3Plan;
        p->fptr_execplan = &execPV3Plan;
        p->fptr_killplan = &killPV3Plan;
        p->fptr_perfplan = &perfPV3Plan;
        p->name = PV3;
        d = (PV3data *)malloc(sizeof(PV3data));
        assert(d);
        if(d){
            if(m->isize == 1){
                d->M = m->i[0] / sizeof(double);
            } else {
                d->M = m->d[0] / sizeof(double);
            }
        }
        (p->vptr) = (void *)d;
    }
...
/**
 * \brief A power hungry streaming computational algorithm on one array of 64bit values, which will operate with a memory footprint of "size" bytes. This load was tuned to a quadcore AMD "Instabul" processor, but may be suitable for loading multiple x86-64 cores until the memory system is saturated. It is intended to be run with a footprint large enough to require main memory access.
...
int perfPV3Plan(void *plan){
    ...
    if(p->exec_count > 0){
        ...
        EmitLogfs(MyRank, 9999, "PV3 plan performance:", trips, "Million Trips/s", PRINT_SOME);
        EmitLogfs(MyRank, 9999, "PV3 plan performance:", mbps,  "MB/s",   PRINT_SOME);
        EmitLog  (MyRank, 9999, "PV3 execution count :", p->exec_count, PRINT_SOME);
        ret = ERR_CLEAN;
    }
    return ret;
} /* perfPV3Plan */
 ...
 * \sa killPV3Plan
 */
int parsePV3Plan(char *line, LoadPlan *output){
