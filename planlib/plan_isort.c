...
*/
void
isort_swap (uint64_t * array, int idx1, int idx2)
{
  uint64_t t;
  t = array[idx1];
...
*/
int
isort_partition (uint64_t * array, int left, int right, int pivot_index)
{
  int i, st_index;
  uint64_t pivot_val;
...
*/
int
isort_quicksort (uint64_t * array, int left, int right)
{
 ...
 */
int
initISORTPlan (void *plan)
{				// <- Replace ISORT with the name of your module.
  if (!plan)
  ...
  if (p)
      ...
      if (DO_PERF)
	{
	  ...
	  TEST_PAPI (PAPI_create_eventset (&p->PAPI_EventSet), PAPI_OK,
		     MyRank, 9999, PRINT_SOME);
	  ...
	  for (i = 0; i < TOTAL_PAPI_EVENTS && i < NUM_PAPI_EVENTS; i++)
		...
		{
		  p->PAPI_Num_Events++;
		  TEST_PAPI (PAPI_add_event (p->PAPI_EventSet, temp_event),
			     PAPI_OK, MyRank, 9999, PRINT_SOME);
		}
	    }
	  ...
	  TEST_PAPI (PAPI_start (p->PAPI_EventSet), PAPI_OK, MyRank, 9999,
		     PRINT_SOME);
#endif //HAVE_PAPI
        }				//DO_PERF
    }
...
{				// <- Replace ISORT with the name of your module.
  ...
  if (DO_PERF)
    {
      ...
      TEST_PAPI (PAPI_reset (p->PAPI_EventSet), PAPI_OK, MyRank, 9999,
		 PRINT_SOME);
  ...
  if (DO_PERF)
    {
      ...
      TEST_PAPI (PAPI_accum (p->PAPI_EventSet, p->PAPI_Results), PAPI_OK,
		 MyRank, 9999, PRINT_SOME);
  ...
  if (CHECK_CALC)
    {				// Evaluates to true if the '-t' option is passed on the commandline.
      if (DO_PERF)
	{
	  ORB_read (t1);
	}			//DO_PERF
...
killISORTPlan (void *plan)
{
  ...
  if (DO_PERF)
    {
#ifdef HAVE_PAPI
      TEST_PAPI (PAPI_stop (p->PAPI_EventSet, NULL), PAPI_OK, MyRank, 9999,
		 PRINT_SOME);
#endif //HAVE_PAPI
    }				//DO_PERF
 ...
 */
int
parseISORTPlan (char *line, LoadPlan * output)
{				// <- Replace ISORT with the name of your module.
...
perfISORTPlan (void *plan)
{
  ...
  d = (ISORTdata *) p->vptr;
  if (p->exec_count > 0)
    {				// Ensures the plan has been executed at least once...
      ...
      EmitLog (MyRank, 9999, "YOUR_PLAN execution count :", p->exec_count,
	       PRINT_SOME);
      ret = ERR_CLEAN;
    }
  return ret;
